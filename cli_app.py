...
"{", ".join(missing_params)}"\nYou can use natural language or any format to give these values.
            ...
            for param in missing_params:
                toolargstemp[param] = Field(
                    f"""It is a parameter of function {tool}. Name of parameter is {param} and its datatype is {func_params[param].annotation}. Please return "nill" if that required value of that parameter is missing in user query or context."""
                )
                param_list += f"{param}: {func_params[param].annotation}"
...
if __name__ == "__main__":
    while True:
        ...
        print(
            f"\n\nSummary of steps taken by Agent: \n{state['final_report'].actionAnalysis}\n\n"
        )

        print(f"Response of AI:\n {state['final_report'].finalResponse}\n")
